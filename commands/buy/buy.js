...
module.exports = {
    ...
    run : async(client, message, args) => {
		...
		console.log(price);
		if(!price) return message.reply("This player is not available in the market");
		if(arr.find(m => m.id == playerToBuy)) {
			if(balance >= price.price) {
				...
				if(val) {
					...
					message.reply(`You have succesfully bought ${price.name} for $${nm("#,###.", price.price)}`)
					console.log("Bought");
					client.db.push(`cards-${message.author.id}`, price);
				}
				} else {
					message.reply(`Sorry, you do not have enough money to buy this player. You need $${nm("#,###.", price.price - balance)} more!`);
